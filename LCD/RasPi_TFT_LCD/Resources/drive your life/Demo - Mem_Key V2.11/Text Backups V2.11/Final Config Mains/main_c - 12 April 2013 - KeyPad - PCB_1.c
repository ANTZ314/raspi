...
/* Filename	: main.c	    		                                        	*/
/*                                                                      		*/
/* Brief description : 	Processor: AT32UC3A0512-AU								*/
/*						Could use any AT32 - (RAM - 11Kb) (flash - 35.8 Kb)		*/
/*						Audio: .RAW file - (44100, STEREO, 16BIT BIG ENDIAN)	*/
/*						Image: .bmp file - scaled to LCD (480x272)				*/
/*						LCD (Nu-Vision): 480x272 4.3'							*/
/*                      Membrane Keypad (Electronic Touch Systems)         		*/
/*                                                                      		*/
 ...
 * -> Image display and menu navigation working (incorrect keypad)
 * -> PIN defines changed to fit new Layout
 * -> Added 'soft-start' hardware to circuit
 
 * MUST FIX:
 * => Include membrane Keypad Routine...
 *		=> change keypad defines to new layout pins (user_board.h)
 * => (manual volume control) -> Digital Volume Control
 * => I/O initialize to faster screen update
 * => 12MHz Crystal doesn't seem to be used (independent clock for LCD ?)
 * => Add 'soft-start' routine at beginning
...
unsigned short					timer				= 0;		// counter for timer
...
void init_IOs(void)
{
	...
	// Enable as input pin (longer than 1 cycle)
	gpio_enable_pin_glitch_filter(COL1);
	gpio_enable_pin_glitch_filter(COL2);
	gpio_enable_pin_glitch_filter(COL3);
	
/*	gpio_enable_gpio_pin(COL1);
	gpio_clr_gpio_pin(COL1);
	gpio_enable_gpio_pin(COL2);
	gpio_clr_gpio_pin(COL2);
	gpio_enable_gpio_pin(COL3);
	gpio_clr_gpio_pin(COL3);*/
	...
	gpio_enable_gpio_pin(ROW1);	
	gpio_set_gpio_pin(ROW1);
	gpio_enable_gpio_pin(ROW2);	
	gpio_set_gpio_pin(ROW2);
	gpio_enable_gpio_pin(ROW3);	
	gpio_set_gpio_pin(ROW3);
	gpio_enable_gpio_pin(ROW4);	
	gpio_set_gpio_pin(ROW4);
...
void blink_led1(void)							// Red
{
	for (int x=0; x<=2; x++)
	{
		gpio_tgl_gpio_pin(LED1);
		
		for (timer = 0; timer < 25; timer++)	// b<25
		{
			wait();
		}
	}	
}
void blink_led2(void)							// Green
{
	for (int x=0; x<=2; x++)
	{
...
void soft_start(void)
{
	// HOW DID WAYNE DO THE POWER OFF FROM POWER BUTTON ? ? ?
	
	// wait 2sec
	for (timer = 0; timer < 35; timer++)		// b<25
	{
		wait();
	}
	gpio_set_pin_high(on_off);					// Power pin (High = ON)
}
void power_off(void)
{	
	//Display goodbye -> wait 2sec
	// wait 2sec
	for (timer = 0; timer < 35; timer++)		// b<25
	{
		wait();
	}
	gpio_set_pin_low(on_off);
}
//===================================================
//********************* Main ************************
//===================================================
...
{	
	...
	//gpio_local_init();					// FASTER LED MODE FOR GENERAL USE...
	init_IOs();
	...
	// Check if power button held for +/-2sec
	soft_start();
	...
	// Initialize SD-Card, SSC module, SPI and PDCA options
	sd_reader_init();
	
	
	gpio_set_pin_high(LED1);				// RED LED on - initialized successfully
	gpio_set_pin_low(Bl_En);				// Back light off
	
	/*im_no = 0;								// Intro Screen
	Get_Image_file(filename1, im_no);
...
//  Audio_Decode(filename2);
	...
	// Wait for button press:				COL1/ROW4 -> No.2
	gpio_set_pin_low(ROW2);					// column1 = pull down
	while(button != 0x00)
	{
		// when btn pressed COL1 will become low
		button = gpio_get_pin_value(COL1);
	}			
	gpio_set_pin_high(ROW2);				// column1 = pull up
	button = true;
	...
	mode	= 0;							// Learn-(mode 0) / Test-(mode 1)
	menu	= 0;							// Main Menu (Main-0 / Sub-1)
	...
	while (1)
	{
		...
		button = gpio_get_pin_value(COL2);
		if (button != 0x00)
		{
			blink_led2();					// red
		}			
...
void Get_Image_file(char * buf, int choice)
 {
	...
	unsigned char Ans6[22]		= "A:/Folder1\\File20.bmp";
	...
	{
	case 0:									// START SCREEN
		while (k<22){folder[k] = Intro[k];k++;}
		break;
	case 1:									// Menu grid Image 1
		while (k<22){folder[k] = Menu1[k];k++;}
		break;
	case 2:									// Menu grid Image 2
		while (k<22){folder[k] = Menu2[k];k++;}
		break;
	case 3:									// Menu grid Image 3
		while (k<22){folder[k] = Menu3[k];k++;}
		break;
	case 4:									// Menu grid Image 4
		while (k<22){folder[k] = Menu4[k];k++;}
		break;
	case 5:									// Menu grid Image 5
		while (k<22){folder[k] = Menu5[k];k++;}
		break;
	case 6:									// Menu grid Image 6
		while (k<22){folder[k] = Menu6[k];k++;}
		break;
	case 7:									// Menu grid Image 7
		while (k<22){folder[k] = Menu7[k];k++;}
		break;
	case 8:									// Menu grid Image 8
		while (k<22){folder[k] = Menu8[k];k++;}
		break;
	case 9:									// Menu grid Image 9
		while (k<22){folder[k] = Menu9[k];k++;}
		break;
	case 10:								// Menu grid Image 10
		while (k<22){folder[k] = Menu10[k];	k++;}
		break;
	case 11:								// Menu grid Image 11
		while (k<22){folder[k] = Menu11[k];	k++;}
		break;
	case 12:								// Menu grid Image 12
		while (k<22){folder[k] = Menu12[k];	k++;}
		break;
/************************************************************************/
	case 21:								// Answer Image 1
		while (k<22){folder[k] = Ans1[k];k++;}
		break;
	case 22:								// Answer Image 2
		while (k<22){folder[k] = Ans2[k];k++;}
		break;
	case 23:								// Answer Image 3
		while (k<22){folder[k] = Ans3[k];k++;}
		break;
	case 24:								// Answer Image 4
		while (k<22){folder[k] = Ans4[k];k++;}
		break;
	case 25:								// Answer Image 5
		while (k<22){folder[k] = Ans5[k];k++;}
		break;
	case 26:								// Answer Image 6
		while (k<22){folder[k] = Ans6[k];k++;}
		break;
	case 27:								// Answer Image 7
		while (k<22){folder[k] = Ans7[k];k++;}
		break;
	case 28:								// Answer Image 8
		while (k<22){folder[k] = Ans8[k];k++;}
		break;
	case 29:								// Answer Image 9
		while (k<22){folder[k] = Ans9[k];k++;}
		break;
	case 30:								// Answer Image 10
		while (k<22){folder[k] = Ans10[k];k++;}
		break;
	case 31:								// Answer Image 11
		while (k<22){folder[k] = Ans11[k];k++;}
		break;
	case 32:								// Answer Image 12
		while (k<22){folder[k] = Ans12[k];k++;}
		break;
/************************************************************************/			
	case 41:								// Question Image 1
	case 44:								// Question Image 4
	case 47:								// Question Image 7
	case 50:								// Question Image 10
		while (k<22){ folder[k] = Quest1[k]; k++; }
		break;
	case 42:								// Question Image 2
	case 45:								// Question Image 5
	case 48:								// Question Image 8
	case 51:								// Question Image 11
		while (k<22){folder[k] = Quest2[k];	k++;}
		break;
	case 43:								// Question Image 3
	case 46:								// Question Image 6
	case 49:								// Question Image 9
	case 52:								// Question Image 12
		while (k<22){folder[k] = Quest3[k];	k++;} 
		break;
/************************************************************************/				
	case 98:								// Main Menu 1
		while (k<22){folder[k] = M_Menu1[k];k++;}
		break;
	case 99:								// Main Menu 2
		while (k<22){folder[k] = M_Menu2[k];k++;}
		break;
...
void Get_Audio_file(char * buf, int choice)
 {
	...
	// ssc_i2s_init(ssc, 44100, 16, 16, SSC_I2S_MODE_STEREO_OUT, FPBA_HZ);
	// convert audio file - (44100, STEREO, 16BIT BIG ENDIAN)
...
/*
	...
	unsigned char Audio15[22] = "A:/Folder2\\File16.raw";		// NoHitchHiking
	unsigned char Audio15[22] = "A:/Folder2\\File17.raw";		// NoHitchHiking+QT
	unsigned char Audio15[22] = "A:/Folder2\\File18.raw";		// Headlamps
	unsigned char Audio15[22] = "A:/Folder2\\File19.raw";		// Headlamps+QT
	unsigned char Audio15[22] = "A:/Folder2\\File20.raw";		// FallingRocks
	unsigned char Audio15[22] = "A:/Folder2\\File21.raw";		// FallingRocks+QT
	unsigned char Audio15[22] = "A:/Folder2\\File22.raw";		// NoOvertaking
	unsigned char Audio15[22] = "A:/Folder2\\File23.raw";		// NoOvertaking+QT*/
	...
	switch(choice)
	{
		case 0:													// File1.raw (Intro Track)
		while (k<22){ folder1[k] = IntroAu[k]; k++; }
					break;
		case 21:												// File00.raw (Learn)
		while (k<22){ folder1[k] = Audio0[k]; k++; }
					break;
		case 41:												// File01.raw (Test)
		while (k<22){ folder1[k] = Audio1[k]; k++; }
					break;
		case 22:												// File02.raw (Learn)
		while (k<22){ folder1[k] = Audio2[k]; k++; }
					break;
		case 42:												// File03.raw (Test)
		while (k<22){ folder1[k] = Audio3[k]; k++; }
					break;
		case 23:												// File04.raw (Learn)
		while (k<22){ folder1[k] = Audio4[k]; k++; }
					break;
		case 43:												// File05.raw (Test)
		while (k<22){ folder1[k] = Audio5[k]; k++; }
					break;
		case 24:												// File06.raw (Learn)
		while (k<22){ folder1[k] = Audio6[k]; k++; }
					break;
		case 44:												// File07.raw (Test)
		while (k<22){ folder1[k] = Audio7[k]; k++; }
					break;
		case 25:												// File08.raw (Learn)
		while (k<22){ folder1[k] = Audio8[k]; k++; }
					break;
		case 45:												// File09.raw  (Test)
		while (k<22){ folder1[k] = Audio9[k]; k++; }
					break;
		case 26:												// File10.raw (Learn)
		while (k<22){ folder1[k] = Audio10[k]; k++; }
					break;
		case 46:												// File11.raw (Test)
		while (k<22){ folder1[k] = Audio11[k]; k++; }
					break;
		case 27:												// File12.raw (Learn)
		while (k<22){ folder1[k] = Audio12[k]; k++; }
					break;
		case 47:												// File13.raw (Test)
		while (k<22){ folder1[k] = Audio13[k]; k++; }
					break;
		case 28:												// File14.raw (Learn)
		while (k<22){ folder1[k] = Audio14[k]; k++; }
					break;
		case 48:												// File15.raw (Test)
		while (k<22){ folder1[k] = Audio0[k]; k++; }
					break;
		case 29:												// File16.raw (Learn)
		while (k<22){ folder1[k] = Audio1[k]; k++; }
					break;
		case 49:												// File17.raw (Test)
		while (k<22){ folder1[k] = Audio2[k]; k++; }
					break;
		case 30:												// File18.raw (Learn)
		while (k<22){ folder1[k] = Audio3[k]; k++; }
					break;	
		case 50:												// File19.raw (Test)
		while (k<22){ folder1[k] = Audio4[k]; k++; }
					break;
		case 31:												// File20.raw (Learn)
		while (k<22){ folder1[k] = Audio5[k]; k++; }
					break;
		case 51:												// File21.raw (Test)
		while (k<22){ folder1[k] = Audio6[k]; k++; }
					break;
		case 32:												// File22.raw (Learn)
		while (k<22){ folder1[k] = Audio7[k]; k++; }
					break;	
		case 52:												// File23.raw (Test)
		while (k<22){ folder1[k] = Audio8[k]; k++; }
					break;
		default:												// LOUDER.raw (intro)
		while (k<22){ folder1[k] = IntroAu[k]; k++; }
					break;	
	}
...
int Audio_Decode(char * path)
{
	...
	volatile bool			esc			= true;
	
	// Pin low for membrane keypad (R3 - C1 => EXIT)
	// Pin High again at end of the function
	gpio_set_pin_low(ROW3);
...
//=========================================================================
			...
			// reset counter encase of very large file (not fully necessary except type defined)
			if (Start_Tx == 999){ Start_Tx = 3;	}			
		   
			size -= (i * FS_SIZE_OF_SECTOR);					// Decrease remaining file size
			in_buffer = !in_buffer;								// invert value
			
			// Check for 'EXIT' key (C1 - R3)
			// pulled low start -> high again before end function
			// Must press and hold untill function reaches this point
			esc = gpio_get_pin_value(COL1);						// check if EXIT button pressed
			if (esc == false)									// move image 
			{
				in_transferTx = false;							// disable PDCA transfer
				//stop_play();									// stop audio playback & data transfer
				gpio_set_pin_high(ROW3);						// 'EXIT' key high again
				free(ptrFile);									// free the Buffer
				close(fd);										// Close file
...
//=========================================================================
			...
			// Output last 'size' data to PDCA
			// depending which buffer is left
			// 0 -> 'size' copied to ram buffers
				if (!in_buffer)
				{
					// wait for Tx1 to finish -> 1 = bizy
					while (Tx_buff1) ;	
...
//=========================================================================
   ...
   // Close file
   close(fd);											// fd = '0'
   gpio_set_pin_high(ROW3);								// keypad cross pin (set low at function start)
