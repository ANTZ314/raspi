		...
		switch(ROW_NO)
			{
				case 0:
						...
						if(!row_pin){LEFT_PRESSED_TRUE; KEYPAD_PRESSED_TRUE; while(!ROW2);}
						row_pin = gpio_get_pin_value(COL3);
						if(!row_pin){Q3_PRESSED_TRUE; KEYPAD_PRESSED_TRUE; while(!ROW3);}
						gpio_set_pin_high(ROW1);					//column1 = release
						break;
				case 1:
						gpio_set_pin_low(ROW2);						//column2 = pull down
						row_pin = gpio_get_pin_value(COL1);	
						if(!row_pin){ENTER_PRESSED_TRUE; KEYPAD_PRESSED_TRUE; while(!ROW1);}
						row_pin = gpio_get_pin_value(COL2);	
						if(!row_pin){TOP_PRESSED_TRUE; KEYPAD_PRESSED_TRUE; while(!ROW2);}
						row_pin = gpio_get_pin_value(COL3);	
						if(!row_pin){BOTTOM_PRESSED_TRUE; KEYPAD_PRESSED_TRUE; while(!ROW3);}
						gpio_set_pin_high(ROW2);					//column2 = release
						break;
				case 2:
						gpio_set_pin_low(ROW3);						//column3 = pull down
						row_pin = gpio_get_pin_value(COL1);	
						if(!row_pin){EXIT_PRESSED_TRUE; KEYPAD_PRESSED_TRUE; while(!ROW1);}
						row_pin = gpio_get_pin_value(COL2);	
						if(!row_pin){RIGHT_PRESSED_TRUE; KEYPAD_PRESSED_TRUE; while(!ROW2);}
