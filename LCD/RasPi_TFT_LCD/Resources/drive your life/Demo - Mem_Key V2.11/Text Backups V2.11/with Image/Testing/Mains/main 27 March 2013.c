...
/* Filename	: main.c	    		                                        	*/
/*                                                                      		*/
/* Brief description : 	Processor: AT32UC3A0512-AU (could use any AT32)			*/
...
//  Audio_Decode(filename2);
	...
	gpio_set_pin_low(ROW4);					// column1 = pull down
	while(button)
	{
		// when btn pressed COL1 will become low
		button = gpio_get_pin_value(COL1);
	}			
	gpio_set_pin_high(ROW4);				// column1 = pull up
...
//=========================================================================
			...
			// Check for escape (C1 - R3)
			gpio_set_pin_low(ROW3);
			esc = gpio_get_pin_value(COL1);
