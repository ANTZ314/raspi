...
/*
	...
	Write_Data(0x00);				//SET vertical size=272-1 HightByte
	Write_Data(0xef);				//SET vertical size=272-1 LowByte
	Write_Data(0x00);				//SET even/odd line RGB seq.=RGB

// original 18bit -> 6 6 6 format
/*
	Command_Write(0xf0,0x03);   //SET pixel data I/F format = 16  (5 6 5)
	Command_Write(0x3a,0x50);   // SET R G B format = 16bit/pixel (5 6 5)
*/
...
// 	Write_Data(0x0f);
	...
	Write_Command(0x36);
	Write_Data(0x0A);		  //was 0x00 (RAW) -> 0x0A with BMP
...
void Image_call (const char * image, long size1, int pic_call)
{
	...
	{
		//WindowSet(0x0000,0x01Df,0x0000,0x010F);	// (0, 479, 0, 271)
		WindowSet(0x0000,0x013f,0x0000,0x00EF);		// (0, 320, 0, 240)
		Write_Command(0x2c);						// write memory start
		//a = 0x36;									// 54 bytes of header data	
	}	
