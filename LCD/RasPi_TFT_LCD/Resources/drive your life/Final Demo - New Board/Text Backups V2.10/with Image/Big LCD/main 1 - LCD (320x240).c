/*
 * \ file: Audio UserBoard V2.6
 * \ User Board version of "V2_6" for AT32UC3A0512AU
 ...
 * -> Audio Output is correct, with NOISE and POP, -> 16bit - (Big Endian) - Stereo?
 * -> Input Buttons correct
 * -> Image display works and scrolls between menus and images  
 
 * MUST FIX:
 * => Audio quality -> noise and POP
 * => Check speaker output to earphone switch
 * => Image shift (54 bytes on BMP data)
 * => Image screen update mode
 */
#include <asf.h>
#include <avr32/io.h>
#include "compiler.h"
...
//volatile Bool					in_transferRx	= 0;		// stop start SPI Rx
...
{	
	if(in_transferTx) 
	{
		// already did ram_buf1[] on PDC startup
		if (!in_buffer)
		{ 
// 			Tx_buff1 = false;
// 			Tx_buff2 = true;
			pdca_reload_channel( AVR32_PDCA_CHANNEL_SSC_TX, (void *)&ram_buffer2, 2048);	// Tx_size);
			Tx_buff1 = false;
			Tx_buff2 = true;
		} 
		else if (in_buffer)
...
// 			Tx_buff1 = false;
// 			Tx_buff2 = true;
			pdca_reload_channel( AVR32_PDCA_CHANNEL_SSC_TX, (void *)&ram_buffer1, 2048);	// Tx_size);
			Tx_buff1 = true;
			Tx_buff2 = false;
		}
	}			
//===================================================
	...
	// disables from "Get_filename()" -> final data
	
	// Swap between buffers ?? -> in both ?? or seperate buffer keepers
	//in_buffer = !in_buffer;
}
...
void local_pdca_init(void)
{
	...
	{
		.addr			= (void *)&ram_buffer2,	//NULL,	// We don't want to receive anything now.
		.size			= 512,					//0,			
		.r_addr			= NULL,
		.r_size			= 0,
...
// 	bool v = 0;			// EXIT
	...
	// initialize values to ram_buffer 
	for (check = 0; check < 2055; check++)
	{
		ram_buffer1[check] = 0x55;
		ram_buffer2[check] = 0x55;
	}
	gpio_set_pin_high(LED);
	
	sd_reader_init();
	
	im_no = 11;	//0;							// Display Intro Screen
 	Get_Image_file(filename1, im_no);
 	Image_Decode(filename1);
	//FULL_ON(0x22);
...
// 	Image_Decode(filename1);
	...
	Get_Audio_file(filename);
	Audio_Decode(filename);
	...
	while(1)
	{
		...
		else if (t == true)					// move image 
		{
			next_image(1);
			t = false;
		}
		else if (u == true)					// move image 
		{
			next_image(4);
			u = false;
		}
		else if (v == true)					// move image 
		{
			next_image(5);
			v = false;
		}*/
...
void Get_Audio_file(char * buf)
 {
	...
	unsigned char folder[21] = "A:/Folder2\\File3.raw";			// ssc_i2s_init(ssc, 44100, 16, 16, SSC_I2S_MODE_STEREO_OUT, FPBA_HZ);
	//unsigned char folder[21] = "A:/Folder2\\File3.raw";
...
//=========================================================================
			...
			count	= 0x00;
				...
				else if (in_buffer)
				{
					gpio_clr_gpio_pin(TOP);
					// wait for Tx1 to finish -> 1 = bizy
					// interrupt and reload channel for Tx1 -> then release Tx_buff2
					while (Tx_buff2) ;
			...
			// After 2nd buff full -> start SSC Tx
			if(Start_Tx == 2)
			{
				in_transferTx = true;
