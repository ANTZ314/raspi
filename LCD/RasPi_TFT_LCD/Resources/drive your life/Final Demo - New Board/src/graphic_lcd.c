...
// Enable LCD Data Pin Outs
	gpio_local_enable_pin_output_driver(LCD0);
	gpio_local_enable_pin_output_driver(LCD1);
	gpio_local_enable_pin_output_driver(LCD2);
	gpio_local_enable_pin_output_driver(LCD3);
	gpio_local_enable_pin_output_driver(LCD4);
	gpio_local_enable_pin_output_driver(LCD5);
	gpio_local_enable_pin_output_driver(LCD6);
	gpio_local_enable_pin_output_driver(LCD7);
...
// Clear all LCD Data Pin Outs
	gpio_local_clr_gpio_pin(LCD0);
	gpio_local_clr_gpio_pin(LCD1);
	gpio_local_clr_gpio_pin(LCD2);
	gpio_local_clr_gpio_pin(LCD3);
	gpio_local_clr_gpio_pin(LCD4);
	gpio_local_clr_gpio_pin(LCD5);
	gpio_local_clr_gpio_pin(LCD6);
	gpio_local_clr_gpio_pin(LCD7);*/
	
	for (int count = 0; count < 8; count++)
	{
		gpio_enable_gpio_pin(LCD_DB1[count]);		// enable pins separately
		gpio_clr_gpio_pin(LCD_DB1[count]);			// always clear pin before use
	}
	
	gpio_enable_gpio_pin(IC_A0);
	gpio_clr_gpio_pin(IC_A0);
	gpio_enable_gpio_pin(IC_CS);
	gpio_clr_gpio_pin(IC_CS);
	gpio_enable_gpio_pin(IC_RST);
	gpio_clr_gpio_pin(IC_RST);
	gpio_enable_gpio_pin(IC_WR);
	gpio_clr_gpio_pin(IC_WR);
	gpio_enable_gpio_pin(IC_RD);
	gpio_clr_gpio_pin(IC_RD);
