...
def main():
	...
	for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
			...
			if (timestamp - lastUploaded).seconds >= conf["min_upload_seconds"]:
				...
				if motionCounter >= conf["min_motion_frames"]:
					...
					# Otherwise store image in working folder
						...
						# write the image to temporary file
						ts = timestamp.strftime("/home/pi/Pictures/dropbox/%A %d %B %Y %I:%M:%S%p") + ".jpg"
						rotate = imutils.rotate(frame, 180)
						cv2.imwrite(ts, rotate)		# was 'frame'
		...
		if conf["show_video"]:
			# Rotate the display and storage?
			rotate_90(frame, 90)	#frame = np.rot90(frame)
