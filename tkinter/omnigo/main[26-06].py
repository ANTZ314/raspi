...
Changes from main8:
	ID scan time-out period - 20sec
	Auto-start count mode:
		-> remove start button (grey out?)
		-> still requires stop/finish button
...
def main():
	...
	global CodeDone									# code entered flag
	
	CodeDone = False								# negative
		...
		try:
			...
			def code(value):
				...
				## Any digit keys pressed ##
					...
					#e.insert('end', value)					# add number to `entry`
			...
			def KeyPadExit(CodeDone):
				...
				print("Destroy Window...")					# console - REMOVE
				keyPadWin = lay[0]
				
				if CodeDone == True:
					print("Quit Main Program!!")			# console - REMOVE
					## Destroy All Windows ##
					win.quit()
					win.destroy()
				else:
					...
					print("[INFO] Exit Code Incorrect")		# consle - REMOVE
